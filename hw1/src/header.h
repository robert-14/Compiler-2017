...
typedef struct Value{
    ...
    union{
        //char id;
        char id[65];  //2017.10.1                 /* if the node represent the access of the identifier */
...
typedef struct Statement{
    StmtType type;
    union{
        char variable[65];//2017.10.1              /* print statement */
...
Statement makeAssignmentNode( char *id, Expression *v, Expression *expr_tail );//2017/10/2
Statement makePrintNode( char *id );                                            //2017.10.2
Statements *makeStatementTree( Statement stmt, Statements *stmts );
Statement parseStatement( FILE *source, Token token );
Statements *parseStatements( FILE * source );
Program parser( FILE *source );
void InitializeTable( SymbolTable *table );
void add_table( SymbolTable *table, char *c, DataType t );//2017.10.2
SymbolTable build( Program program );
void convertType( Expression * old, DataType type );
DataType generalize( Expression *left, Expression *right );
DataType lookup_table( SymbolTable *table, char *c );   //2017.10.2
