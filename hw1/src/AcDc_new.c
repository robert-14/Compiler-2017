...
Declarations *parseDeclarations( FILE *source )
{
    ...
    switch(token.type){
        ...
        case Alphabet:
            ...
            //printf("unget token is %s\n",token.tok);
            ungetc(' ', source);    
            for(int i = strlen(token.tok) - 1; i >= 0 ; i--){ 
                //printf("unget char is %c\n",token.tok[i]);
    ...
    switch(token.type){
        case Alphabet:
            ...
            if(next_token.type == AssignmentOp){
                ...
                return makeAssignmentNode(token.tok, value, expr);//2017.10.2
            }
            else{
        ...
        case PrintOp:
            ...
            if(next_token.type == Alphabet)
                return makePrintNode(next_token.tok);//2017.10.2
            else{
...
Statements *parseStatements( FILE * source )
{
    ...
    switch(token.type){
        ...
        case EOFsymbol:
            return NULL;
        default:
            printf("Syntax Error: statements %s\n", token.tok);
            exit(1);
    }
}
...
void add_table( SymbolTable *table, char *c, DataType t )
{
    ...
    if(!flag){
        strcpy(table->table2[table->num], c);
        (table->num)++;
    }
